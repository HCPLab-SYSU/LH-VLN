...
def dist_models(args, model, logger):
    ...
    device_id = args.rank % total_gpus# torch.device("cuda:0" if torch.cuda.is_available() else "cpu")# 
    ...
    for n, p in model.named_parameters():
        ...
        logger.info(f"layer name {n}, layer grad {p.requires_grad}")

    # frozens = ['model.layers.0.', 'model.layers.1.', 'model.layers.2.', 'model.layers.3.', 'model.layers.4.']
    # for n, p in model.named_parameters():
